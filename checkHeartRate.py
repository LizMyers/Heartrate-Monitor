...
# batLevel = bat.communicate("get battery")
# result = batLevel[0] #result: 'battery: 24.5678\n'
# numb = float(result[9:-1])
...
#IMPORTANT: Install PiSugar2 Server (on pi-zero) w/this command: curl http://cdn.pisugar.com/release/Pisugar-power-manager.sh | sudo bash

#DOCS: https://github.com/PiSugar/PiSugar/wiki/PiSugar-Power-Manager-(Software)

def getBattery():
    """returns a String from server API (example: battery: 32.6789)"""
    bat = subprocess.Popen("nc -q 0 127.0.0.1 8423", 
    stdin=subprocess.PIPE, stdout=subprocess.PIPE, 
    stderr=subprocess.PIPE, shell=True)
    batLevel = bat.communicate("get battery")
    result = batLevel[0] #first elem in tupple
    numb = float(result[9:-1])
    percent = int(round(numb))
    battery = str(percent) + "%\n"
    return battery 

def printToDisplay(bpm, cTime, battery):
    font40 = ImageFont.truetype(('Vera.ttf'), 40)
    font22 = ImageFont.truetype(('Vera.ttf'), 22)
    image = Image.new('1', (epd.height, epd.width), 255)
    draw = ImageDraw.Draw(image)
    draw.text((10, 14), bpm, font = font40, fill = 0)
    draw.text((15, 32), cTime, font = font22, fill = 0)
    draw.text((15, 58), battery, font = font22, fill = 0)
    epd.display(epd.getbuffer(image)) #display txt & lines
...
def main():
    ...
    while True:
      ...
      jwt_refresh = time.time() + ((token_life - 1) * 360) #set a refresh time for one minute before the JWT expires
            ...
            if inputReceived == 1:
                if previousInput == 0: # the heart beat signal went from low to high
                    ...
                    if sampleCounter == heartbeatsToCount: # time to calculate the average BPM
                        ...
                        print("{}\n".format(payload))
                        #----------------------------------
                        #Uncomment next 3 lines for display + battery
                        battery = getBattery() 
                        prettyBPM = int(round(bpm))
                        printToDisplay("BPM: " + str(prettyBPM), "\nTime: " 
                        + cTime, "\nBattery: " + battery) 
                        #----------------------------------
                        time.sleep(0.5)
